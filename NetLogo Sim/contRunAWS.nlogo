...
to initWorld
  ...
  while [counteri < noVictims] [
    let xrand1 random-pxcor
    let yrand1 random-pycor
    if [pcolor] of patch xrand1 yrand1 = backgroundColor [
      set counteri (counteri + 1)
      create-victims 1 [
        let xy coords (xrand) (yrand) (20)
...
to Setup
  ...
  assignGenes (readDataFromFile ("geneInfo.txt"))
  reset-ticks
end
...
to-report calcFitnessGA
  let timeFitness 1 - ((ticks - rescueTime) / (totalGATicks - rescueTime)) ;percentage of total time needed
